  ...
  function renderTable() {
    ...
    cache
      ...
      .forEach(c => {
        ...
        const pNombre = escapeHtml(c.nombre.split(' ')[0]);
        const diasTexto = dias !== null && dias > 0 ? `hace ${dias} días` : 'hace un tiempo';
        const waMsg = `Hola ${pNombre}, ¿cómo andás? Te escribimos desde JG Barbería porque ${diasTexto} que no te vemos por acá. Cuando quieras renovar el look, acá te esperamos con el lugar. Estamos en Pellegrini 95, de lunes a sábado de 16 a 21 hs.`;
        const waBtnAdmin = telNorm
          ? `<a href="https://wa.me/549${telNorm}?text=${encodeURIComponent(waMsg)}" target="_blank" rel="noopener" class="wa-circle-btn" title="WhatsApp">${waIcon}</a>`
          : '';
